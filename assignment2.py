...
def churn_rate_by_geography_polars(df):
    # computes customer count and churn rate for each geography using Polars
...
def churn_rate_by_geography_pandas(df):
    # computes customer count and churn rate for each geography using Pandas
...
# ML Modeling: Decision Tree to predict churn
...
def plot_churn_by_age(df, save_path="churn_by_age.png"):
    ...
    # Bar chart of churn rate by age bucket to show who churns most
...
def run_polars_pipeline(path):
    ...
    # runs the full EDA, filter/group-by pipeline with Polars
...
def run_pandas_pipeline(path):
    # runs the full EDA, filter/group-by pipeline with Pandas
